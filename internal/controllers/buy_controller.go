...
func (bc *BuyController) HandleBuy(c *gin.Context) {
	...
	if err != nil {
		// Render error on the buy page (or a dedicated error page)
		renderTemplate(c, "buy.html", gin.H{
	...
	// Maybe redirect to a success page or dashboard with a success message
	renderTemplate(c, "buy_success.html", gin.H{ // Assuming a buy_success.html template exists
